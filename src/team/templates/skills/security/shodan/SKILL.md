...
when-to-use: "Use when probing internet-facing infrastructure (open ports, services, banners) for a target IP, domain, or organization."
metadata:
  flopsy:
...
> **For "how many X are exposed" / "show me the state of X" queries, ALSO read `/skills/recon-discipline/SKILL.md`.** That skill encodes the multi-query triangulation, name-collision check, and output shape that distinguishes a real recon answer from a one-liner. This file documents Shodan-specific tools; recon-discipline documents the broader methodology.
