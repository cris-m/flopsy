...
when-to-use: "Use when the user wants to generate an image, audio, or video clip."
metadata:
  flopsy:
    agent-affinity: [sam]
---
...
```
CHECK → WRITE → RUN → VERIFY → DELIVER
```

1. **Check** API keys: run `[ -n "$OPENAI_API_KEY" ] && echo 'set' || echo 'no'`
2. **Write** script: `write_file("/workspace/work/code/<name>.py", ...)`
3. **Run** via `uv` — deps resolve automatically, no venv setup:
   ```
   execute("uv run --with openai --with requests python /workspace/work/code/<name>.py")
   ```
4. **Verify**: run `wc -c <output-path>` — 0 bytes = failure. Use the right target:
   - Image / chart → `/workspace/work/images/<name>.png`
   - PDF / DOCX / HTML → `/workspace/work/exports/<name>.pdf`
   - CSV (deliverable) → `/workspace/work/exports/<name>.csv`
5. **Deliver**: `send_message({ media: [{ type: "<type>", url: "/workspace/work/<type-dir>/<output>" }] })`

**Never skip step 4.** Delivering a nonexistent file is a silent failure.
...
out = "/workspace/work/images/image.png"
...
out = "/workspace/work/images/chart.png"
...
out = "/workspace/work/exports/report.pdf"
pdf.output(out)
print(f"Saved {os.path.getsize(out)} bytes → {out}")
```
...
out = "/workspace/work/exports/report.docx"
doc.save(out)
print(f"Saved {os.path.getsize(out)} bytes → {out}")
```
...
out = "/workspace/work/exports/results.csv"
...
send_message({
  ...
  media: [
    { type: "image", url: "/workspace/work/images/chart.png" },
    { type: "document", url: "/workspace/work/exports/report.pdf" }
  ]
})
```
...
| `ModuleNotFoundError` | Bare `python` instead of `uv run` | Re-run with `uv run --with <pkg> python …` |
| `_tkinter` crash | Missing `matplotlib.use("Agg")` | Must be called before `import matplotlib.pyplot` |
| `FileNotFoundError` | Writing outside the work tree | Use `/workspace/work/<type>/` — those dirs are pre-created |
| Same error 3x | Stuck on same approach | **Stop.** Switch library or method entirely |

## Lessons Learned
- Split long text into shorter chunks before generating audio — CPU-bound generation times out on long passages
- When audio generation fails on length, immediately retry with smaller segments rather than one monolithic block
