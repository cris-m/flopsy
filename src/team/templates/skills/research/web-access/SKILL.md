...
when-to-use: "Use when the user asks about current events, recent prices, fresh releases, or live data that needs the open web — quick lookup, not deep research."
metadata:
  flopsy:
    agent-affinity: [gandalf, legolas]
---
...
- **In Flopsy specifically:** X/Twitter is served by the `twitter_*` MCP tools (`twitter_read`, `twitter_user_tweets`, `twitter_search`, `twitter_extract`, `twitter_news`). If a `twitter_*` call fails, do NOT re-run it or assume the data is gone — first verify the account is connected (`flopsy auth twitter` / `connect_service`); if it genuinely can't be reached, fall back to `web_search` for a public summary and say so.
...
4. **Access-restricted content** (login walls, paywalls, private or platform-gated social posts): no fetch tool can reach these. Once the strategies above genuinely fail on such a source, ask the user to **paste or describe** the content, then treat their text as the single source of truth (never invent it). This is a LAST resort, not an opening move.
5. Report what was attempted and the specific errors.

**Never open with** "I can't access URLs" / "I'm unable to view that content" / "Could you share the content?" — exhaust the fetch strategies first. Asking for a paste is only appropriate after they fail on genuinely access-restricted content (step 4).
