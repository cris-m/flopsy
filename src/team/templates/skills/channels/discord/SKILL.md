...
when-to-use: "Use when composing a reply on the Discord channel — covers Discord markdown, mentions, and message limits."
category: channels
metadata:
  flopsy:
...
send_message({
  ...
  media: [{ type: "image", url: "/workspace/work/images/chart.png" }]
})
```
...
- Buttons stay visible on the message indefinitely. The bot's window to *respond* to a click closes ~14 minutes after that click (Discord interaction-token limit) — late clicks render "This interaction failed".
...
- Discord renders Markdown automatically — no Telegram-style pre-escape pass. But if you need a *literal* `*`, `_`, `~`, `` ` ``, `|`, `>`, escape it with a backslash (`\*`, `\_`, …) or it will become formatting.
