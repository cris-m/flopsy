...
export const sendPollTool = defineTool({
    name: 'send_poll',
    description: [
        'Send a multiple-choice poll with aggregated voting. Use for "poll", "vote", "survey", or any "pick one of N" request.',
        '',
        'Args:',
        '  question      — ≤300 chars.',
        '  options       — 2-10 choices, each ≤100 chars.',
        '  anonymous     — hide voter identities. Defaults to false so the agent can read vote signals.',
        '  allowMultiple — let voters pick more than one option.',
        '  durationHours — auto-close after N hours. Discord 1-768h; Telegram caps near 10 minutes.',
        '',
        'Native on Telegram and Discord; other channels fall back to numbered text ("1. A / 2. B") where users reply with the number.',
    ].join('\n'),
    schema: z.object({
