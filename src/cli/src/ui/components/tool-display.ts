 ...
 */
export function formatToolResultLines(result: string, termWidth: number, isError: boolean): string[] {
    const dim = chalk.hex(palette.muted);
...
/** Detect error result from text heuristic. */
export function isToolError(result: string | undefined): boolean {
    return !!result && /^\s*(error|exception|traceback|✗|❌|failed?)/i.test(result);
}